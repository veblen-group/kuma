...
use crate::{
    chain::ChainInfo,
    tycho::{ChainSpecificAssetState, state_update::AssetStateUpdate},
};
...
impl ArbitrageStrategyWorker {
    ...
    async fn calculate_slow_chain_amounts(
        &self,
        slow_state: &AssetStateUpdate,
    ) -> eyre::Result<Vec<SlowChainCalculation>> {
...
mod tests {
    ...
    fn test_bidirectional_arbitrage_calculations() {
        ...
        for i in 1..=worker.binary_search_steps {
            ...
            let expected_min_amount =
                (&amount_in * BigUint::from(9925u64)) / BigUint::from(10000u64);
    ...
    fn test_arbitrage_calculation_with_mock_protocol_state() {
        ...
        let slow_state_update =
            create_asset_state_update_with_uniswap_state(&token_a, &token_b, "A");
        let fast_state_update =
            create_asset_state_update_with_uniswap_state(&token_a, &token_b, "A");

        // Test slow chain amount calculation
        let slow_calculations_result =
            tokio_test::block_on(worker.calculate_slow_chain_amounts(&slow_state_update));
    ...
    fn test_profitable_arbitrage_opportunity_detection() {
        ...
        let worker = ArbitrageStrategyWorker {
            ...
            max_trade_amount: BigUint::from(5_000u64), // Much smaller trade size (1000 units, realistic for testing)
            binary_search_steps: 12,                   // More steps for better optimization
        ...
        let slow_state_update =
            create_asset_state_update_with_uniswap_state(&token_a, &token_b, "B");
        let fast_state_update =
            create_asset_state_update_with_uniswap_state(&token_a, &token_b, "A");

        println!(
            "slow_state_price a to b: {}",
            slow_state_update
                .state
        ...
        println!(
            "slow_state_price b to a: {}",
            slow_state_update
                .state
                .spot_price(&token_b, &token_a)
                .unwrap()
        );
        println!(
            "fast_state_price: b to a: {}",
            fast_state_update
                .state
                .spot_price(&token_b, &token_a)
                .unwrap()
        ...
        assert_eq!(slow_calculations.len(), 24);
        ...
        for (i, calc) in slow_calculations.iter().enumerate() {
            ...
            let fast_amount_out = worker
                .simulate_swap(
                    &fast_state_update.state,
                    &calc.output_token,
                    &calc.input_token,
                    &calc.amount_out,
                )
                .unwrap();
            let profit = if fast_amount_out > calc.amount_in {
                &fast_amount_out - &calc.amount_in
            } else {
                BigUint::from(0u64)
            };
            let profit_percentage = if calc.amount_in > BigUint::from(0u64) {
                (profit.clone() * BigUint::from(10000u64) / &calc.amount_in)
                    .to_string()
                    .parse::<f64>()
                    .unwrap_or(0.0)
                    / 100.0
        ...
        match opportunity_received {
            Ok(opportunity) => {
                ...
                println!(
                    "      • Trade Amount: {} units",
                    opportunity.optimal_amount_in
                );
                println!(
                    "      • Expected Profit: {} units",
                ...
                println!(
                    "      • Final Amount Out: {} units",
                    opportunity.fast_chain_amount_out
                );
                println!();
                ...
                println!(
                    "      • Slow Chain: {} ({})",
                    opportunity.slow_chain.chain, "Pool B favored - 950K:1M"
                );
                println!(
                    "      • Fast Chain: {} ({})",
                    opportunity.fast_chain.chain, "Pool A favored - 1M:950K"
                );
                println!(
                    "   💱 Assets: {} ↔ {}",
                    opportunity.asset_a.symbol, opportunity.asset_b.symbol
                );
                println!();
                println!("   🧮 Calculation Breakdown:");
                println!(
                    "      • Slow Chain Output: {} units",
                    opportunity.slow_chain_amount_out
                );
                println!(
                    "      • Fast Chain Final: {} units",
                    opportunity.fast_chain_amount_out
                );
                println!(
                    "      • Net Gain: {} units",
                    &opportunity.fast_chain_amount_out - &opportunity.optimal_amount_in
                );
                println!("   ═══════════════════════════════════════");
            }
