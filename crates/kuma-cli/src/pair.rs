...
impl Pair {
    ...
    pub(crate) fn subset_of_vec(&self, tokens: &[Token]) -> bool {
        self.0.iter().all(|token| tokens.contains(token))
    }
}
...
pub(crate) struct PairState {
    ...
    pub(crate) unmodified_pools: Option<Arc<HashMap<String, Box<dyn ProtocolSim>>>>,
    pub(crate) modified_pools: Option<Arc<HashMap<String, Box<dyn ProtocolSim>>>>,
}
    ...
    fn poll_next(
        ...
        match block_poll {
            ...
            Poll::Ready(Some(block)) => {
                let unmodified_pools = block.unmodified_pools.get(&self.pair).cloned();
                let modified_pools = block.modified_pools.get(&self.pair).cloned();
